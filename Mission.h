...
void FileInput(string fileplace,priority_queue &array_task);//读入本地文件，存储到task_arraay中
void run();//run了之后，开始执行各种命令的函数，各种命令都包含在这个大函数里面
void addTask(string fileplace,priority_queue %task_array);
//这个要注意每个任务的开始时间不同，任务名称+开始时间具有唯一性，任务优先级和类型有缺省值
//还有给任务分配唯一的ID
void showTask(priority_queue task_array);//注意自行设计显示方式
void delTask(string fileplace,priority_queue &array_task,int delID);//根据任务id，删除某个指定任务
void clearTask(string fileplace, priority_queue &task_array);//清空任务
