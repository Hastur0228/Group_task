...
void* check(void* arg)
{
	while (1) {
		...
		while (!task_array.empty()) {
			middle = task_array.top();
			if (middle.is_deleted == 0 && signal == 0) 
			{
				aim=middle;
				top_task_time = middle.remind_time;
				signal = 1;
				if (time_cpr(current_time, top_task_time) == 0 || time_cpr(current_time, top_task_time) == 1)
					middle.is_deleted = 1;
			}
			task_array.pop();
			tmp.push(middle);
		...
		while (!tmp.empty()) {
			middle = tmp.top();
			tmp.pop();
			task_array.push(middle);
		}
		if (top_task_time.year == 0)
		{
			sleep(0.2);
			continue;
		}
		else if (time_cpr(current_time, top_task_time) == 0) {//说明当前时间就是最早提醒时间，需要立刻提醒
			system("clear");
			cout << "！！！！！！！！！！！\n";
			cout << "您有任务已到达提醒时间\n";
			cout << "请合理安排你的时间\n";
